...
<script type="text/javascript">
	...
	function setProductCategory(t) {
		...
		//得到下拉列表中的所有option标签数组
		var ops = category.options;
		for ( var i = 0; i < ops.length; i++) {
		//判断哪一个选项中的value值与t(当前要编辑的书)的类别相等
			if (ops[i].value == t) {
				ops[i].selected = true;
				return;
			}
...
<body>
<!--
 <body obload="setProductCategory('${book.category}')">
作者的写法，下拉框显示
 -->
	<form enctype="multipart/form-data" id="userAction_save_do" name="Form1"
		action="${pageContext.request.contextPath }/UpdateBookServlet?id=${book.id}" method="post">
<!-- 	 或者这样传id  -->
