...
public class UserForm {
	...
	*/
	...
	public boolean validate(){
		...
		}else if(!username.matches("\\w{3,8}")){//3-8位字母
			msg.put("username", "用户名由3-8位字母组成");
		}
		
		if("".equals(password)){
			msg.put("password", "密码不能为空");
		}else if(!password.matches("\\d{3,8}")){//密码为3-8位数字
			msg.put("password", "密码为3-8位数字");
		}
		...
		if("".equals(email)){
			...
			msg.put("email", "请输入正确邮箱地址");
		}
		
		if("".equals(birthday)){
			msg.put("birthday", "生日不能位空");
		}else {
		...
		return msg.isEmpty();//map里没有数据返回true
